...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      // #ifdef H5 || APP-PLUS
      if (this.opts[0]) {
        for (var i = this.childs.length; i--; ) if (this.childs[i].name == 'img') break;
        if (i != -1) {
    ...
    methods: {
      ...
      imgLoad(e) {
        ...
        // #endif
        // 加载完毕，取消加载中占位图
        else if ((this.opts[1] && !this.ctrl[i]) || this.ctrl[i] == -1) this.$set(this.ctrl, i, 1);
      },
      ...
      linkTap(e) {
        ...
        if (href) {
          // 跳转锚点
          if (href[0] == '#') this.root.navigateTo(href.substring(1)).catch(() => {});
          // 复制外部链接
